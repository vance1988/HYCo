...
<#--删-->
<#macro delButton value="" icon="trash" class="danger" id="delButton${.now?date?string('yyyyMMddhhmmss')}" onclick="" title="删除" fieldName="">
	<#if fieldName!="">
		<script>
			$(function() {
				$("#${id}").on("click",function() {
					...
					if (chk_value.length == 0) {
						...
						$("#deleteModal${id}Num").text(chk_value.length);
						$('.deleteBtn${id}').on("click",function() {
							eval(${onclick}(chk_value));
						});
					}
				});				
			})
