...
<@ms.content>
		...
		<@ms.modal modalName="managerPageModel" title="主页面绑定">
			 <@ms.modalBody>
				<@ms.form isvalidation=true name="managerPageForm" id="managerPageForm" class="form-inline" action="${base}${baseManager}/managerModelPage/save.do">	
	...
	$(function () {	
		$("#totalAdd").click(function(){
			...
			URL="${base}${baseManager}/manager/save.do";
	 		$(".update").modal();
	 	});
	...
	$(".updateModalBtn").click(function(){
		...
		URL = "${base}${baseManager}/manager/update.do?managerId="+managerId;
		$(".update").modal();
	});
	 ...
	 $("#totalDelete").click(function(){
		...
		$("#deleteButton").click(function(){
		 ...
		 $("#deleteRadio").click(function(){
		 	$.ajax({
			   type: "post",
			   dataType: "json",
			   url:  "${base}${baseManager}/manager/"+radioManagerId+"/delete.do",
		...
		function queryModelPage(modelId,managerModelPageModelId){
			$.ajax({
			   type: "post",
			   dataType: "json",
			   url:  "${base}${baseManager}/managerModelPage/"+modelId+"/getEntity.do",
		...
		$("#saveUrl").click(function(){
			$.ajax({
				type: "post",
				dataType: "json",
				url:  "${base}${baseManager}/managerModelPage/save.do",
				data:$("#managerPageForm").serialize(),
				beforeSend:function(){
					$("#saveUrl").text("保存中");
