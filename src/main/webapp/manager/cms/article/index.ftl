...
function getZtreeId(event,treeId,treeNode){
	if (treeNode.columnType==1) {
		$("#listFrame").attr("src","${base}${baseManager}/cms/article/"+treeNode.categoryId+"/list.do?categoryTitle="+encodeURIComponent(treeNode.categoryTitle));
		...
		$.ajax({ 
			...
			success: function(msg){
				if (msg.result) {
					$("#listFrame").attr("src","${base}${baseManager}/cms/article/add.do?categoryId="+treeNode.categoryId+"&categoryTitle="+encodeURIComponent(treeNode.categoryTitle));
				} else {
					//如果该单篇栏目下存在文章则跳转到文章编辑页
					$("#listFrame").attr("src","${base}${baseManager}/cms/article/"+treeNode.categoryId+"/edit.do?categoryId="+treeNode.categoryId+"&categoryTitle="+encodeURIComponent(treeNode.categoryTitle));
				}
			},
		});
	} else if(treeNode.columnType=="" || treeNode.columnType == undefined){
		$("#listFrame").attr("src","${base}${baseManager}/cms/article/0/list.do");
	}
}
</script>
</body>
