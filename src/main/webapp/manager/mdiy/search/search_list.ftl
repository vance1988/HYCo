<@ms.html5>
	...
	<script type="text/javascript">	
		...
		function deleteSearch(id){
			var URL="${managerPath}/mdiy/search/"+id+"/delete.do"
			...
			$(this).request({url:URL,type:"json",method:"post",func:function(msg) {
				...
				if(msg != 0) {
		    		...
		    		if($("tbody tr").length==0 && msg != 1){
		    			location.href = base+"${baseManager}/mdiy/search/list.do?pageNo="+(msg-1);
					}else{
						location.href = base+"${baseManager}/mdiy/search/list.do?pageNo="+msg;
					}
		    	} else {
					alert("删除搜索失败");
		...
		$(function () {				
			...
			$(".updateSearch").on("click",function(){
				...
				$(this).request({url:"${managerPath}/mdiy/search/"+searchId+"/edit.do",func:function(search) {
