<@ms.html5>
	...
	<@ms.panel>	
		<#if diyForm?has_content>
			<@ms.form name="diyForm" isvalidation=true  action="${managerPath}/mdiy/diyForm/form/update.do">		
				<!--自定义表单名称-->
		...
		<#else>
			<@ms.form  name="diyForm" isvalidation=true action="${managerPath}/mdiy/diyForm/form/save.do">
				<!--自定义表单名称-->
...
<script type="text/javascript">
	...
	function deleteField(fieldId){
		var URL="${managerPath}/mdiy/diyForm/formField/"+fieldId+"/delete.do";
		...
		<#if diyForm?has_content>
			...
			function queryFieldList(diyFormId){
				$.ajax({
				   type: "post",
				   dataType: "json",
				   url: "${managerPath}/mdiy/diyForm/formField/list.do",
			...
			$("body").delegate("#addField","click",function(){
				...
				var url = "${managerPath}/mdiy/diyForm/formField/"+${diyForm.diyFormId}+"/save.do";
			...
			$("body").delegate(".editform","click",function(){
				...
				$("input[name='diyFormFieldFormId']").val("${diyForm.diyFormId}");
				//表单字段更新地址
				var url = "${managerPath}/mdiy/diyForm/formField/update.do";
				$("#fieldForm").attr("action",url);
				//表单字段ID
				...
				//查询字段对应信息的地址
				var URL="${managerPath}/mdiy/diyForm/formField/"+diyFormFieldId+"/edit.do"
