...
$(function(){
	...
	$("body").delegate("#addButton,.addpeopleAddressTree", "click", function(){    
		...
		if(categoryCategoryId==undefined){
			location.href = base+"${baseManager}/category/add.do?modelId=${modelId?default(0)}&modelTitle=栏目&categoryCategoryId=${categoryCategoryId?default(0)}";
			return;
		}
		
		location.href = base+"${baseManager}/category/add.do?modelId=${modelId?default(0)}&modelTitle=栏目&categoryCategoryId="+categoryCategoryId;
	});
...
function editclumnTree(obj){
	...
	location.href = base+"${baseManager}/category/"+categoryId+"/edit.do?modelId=${modelId?default(0)}&modelTitle="+"栏目";
}
