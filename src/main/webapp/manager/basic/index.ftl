		...
		<div class="row">
			...
			<div class="col-md-10" style="margin-top:0;padding:0;margin:0">
				...
				<div class="row" style="margin-top:0; width:100%;;padding:0 0 0 10px;margin:0">
					<iframe src="${base}${baseManager}/basic/0/list.do" style="width:100%;maring:0;padding:0;border:none;height:100%" id="listFrame" target="listFrame" ></iframe>
				</div>
				<!--内容部分结束-->
			...
			var setting = {
				callback: {
					onClick: function(event, treeId, treeNode) {
							if(treeNode.id>0) {
								$("#listFrame").attr("src","${base}${baseManager}/basic/"+treeNode.id+"/list.do?categoryTitle="+encodeURIComponent(treeNode.name));
							}
					}
				},
				view: {
