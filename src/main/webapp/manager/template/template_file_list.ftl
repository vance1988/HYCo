	       ...
	       $(".name").each(function(i){
			...
			$("table").delegate(".editFileBtn","click",function(){
		        ...
		        if(fileName.indexOf(".") >= 0){
		        	location.href = base+"${baseManager}/template/readFileContent.do?fileName="+fileName;
		        } else {
					location.href=base+"${baseManager}/template/showChildFileAndFolder.do?skinFolderName="+fileName;
				}
			});
