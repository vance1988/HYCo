...
<body>
					...
					<@ms.table head=['<input type="checkbox" id="allCheck"/>,10','编号,50','角色名称']>
						<#if listRole?has_content>
					          ...
					          <tr>
			            		...
			            		<td><a href="${base}${baseManager}/role/${role.roleId}/edit.do">${role.roleName?default("匿名")}</a></td>
					          </tr>
				           </#list>
						<#else>
		           			<tr>
					            <td colspan="4" class="text-center">
					            	<a href="${base}${baseManager}/role/add.do"><@ms.nodata/></a>
								</td>
				          	</tr>
		           		</#if>
					</@ms.table>	
	...
	<script type="text/javascript">	
		...
		function deleteRole(roleId){
			//拼接ajax地址
			var URL="${base}${baseManager}/role/"+roleId+"/delete.do";
		...
		function batchDelete(){
			...
			if(ids.length != 0) {
				var URL= "${base}${baseManager}/role/allDelete.do";
				$("#deleteButtonAll").text("删除中");
				$("#deleteButtonAll").attr("disabled",true);
