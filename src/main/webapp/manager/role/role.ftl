		...
		$(function () {
			...
			$("input[name='roleName']").blur(function(){
				...
				if((oldRoleName != 0 && $("#oldRoleName").val() != roleName) || (oldRoleName == 0)){
					$.ajax({
					   type: "post",
					   dataType: "json",
					   url:  "${base}${baseManager}/role/judgeRoleNameExist.do",
			...
			$("#save").click(function(){
				if($("#saveRoleFrom").data('bootstrapValidator').validate().isValid()){
					...
					var URL = "${base}${baseManager}/role/<#if flag == true>save<#else>update</#if>.do";
					$.ajax({
					   type: "post",
					   dataType: "json",
					   url:  URL,
					   ...
					   success: function(msg){
					    	...
					    	$("#save").removeAttr("disabled");
					   	  	...
					   	  	alert("操作失败");
					   	$("#save").removeAttr("disabled");
					   	  	location.href = "${base}${baseManager}/role/queryList.do";
					   }
					});
				}
			});
